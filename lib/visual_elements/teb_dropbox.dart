...
class TebDropBoxData {
  final String id;
  final String displayValue;
  final IconData? icon;

  TebDropBoxData({required this.id, required this.displayValue, this.icon});
}

typedef OnClose<CustomDropBoxData> = void Function(CustomDropBoxData value);

enum TebDropBoxDisplayMode { dialog, modal }

class TebDropBox extends StatefulWidget {
  final BuildContext context;
  final OnClose<TebDropBoxData> onClose;
  final List<TebDropBoxData> data;
  final String title;
  final String buttonText;
  final TebDropBoxDisplayMode displayMode;
  final Color? color;
  ...
  const TebDropBox({
    ...
    this.displayMode = TebDropBoxDisplayMode.dialog,
    this.buttonIcon,
    this.color,
    this.inicialValue,
  }) : super(key: key);
...
class _TebDropBoxState extends State<TebDropBox> {
  TebDropBoxData? _selectedValue;
...
_openModalSelectionList() {
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      ...
      onPressed: () {
        if (widget.displayMode == TebDropBoxDisplayMode.dialog) {
          _openDialog();
        } else {
          _openModalSelectionList();
        }
